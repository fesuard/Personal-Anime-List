...
class AnimeFilter(django_filters.FilterSet):
    ...
    anime_title = django_filters.CharFilter(lookup_expr='icontains', field_name='title', label='Title',
                                            widget=forms.TextInput(
                                                attrs={'class': 'form-control', 'placeholder': 'Anime title'}))
    anime_status = django_filters.ChoiceFilter(widget=forms.RadioSelect(), field_name='status', choices=status_choice,
                                               label='Status')
    anime_type = django_filters.ChoiceFilter(widget=forms.RadioSelect(), field_name='type', choices=type_choice,
                                             label='Type')
    ...
    anime_season = django_filters.ChoiceFilter(widget=forms.RadioSelect(), field_name='season', choices=season_choice,
                                               label='Season')
    anime_tags = django_filters.CharFilter(method='filter_for_search', label='Tags',
                                           widget=forms.TextInput(
...
class AnimeListFilter(django_filters.FilterSet):
    ...
    anime = django_filters.CharFilter(field_name='anime__title', lookup_expr='icontains', label='Title',
                                      widget=forms.TextInput(
                                          attrs={'class': 'form-control', 'placeholder': 'Anime title'}))
    # tags = django_filters.CharFilter(field_name='anime__tags__name', lookup_expr='icontains', label='Tags', widget=forms.TextInput(
    #     attrs={'class': 'form-control', 'placeholder': 'Anime tags'}))
    tags = django_filters.CharFilter(method='filter_for_tags', label='Tags', widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': 'Anime tags'}))
    watch_status = django_filters.ChoiceFilter(widget=forms.RadioSelect(), choices=watch_status_choice,
                                               label='Watch Status')
    score = django_filters.ChoiceFilter(widget=forms.RadioSelect(), choices=score_choice, label='Score')
    show_type = django_filters.ChoiceFilter(widget=forms.RadioSelect(), field_name='anime__type', label='Show Type',
                                            choices=anime_type_choice)
